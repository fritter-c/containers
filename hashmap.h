...
struct hashmap : private _allocator_ebo<hashmap_bucket<Key, T>, Allocator> {
    ...
    inline explicit hashmap(std::size_t _Reserved)
        : _allocator_ebo<hashmap_bucket<Key, T>, Allocator>(), size(0), capacity(first_power2(_Reserved)) {
    ...
    inline hashmap(std::initializer_list<std::pair<Key, T>> init_list) : _allocator_ebo<hashmap_bucket<Key, T>, Allocator>() {
        capacity = first_power2(init_list.size() * 4);
        size = 0;
        data = this->malloc(capacity);
        std::memset((char *)(void *)data, 0, capacity * bucket_size());
    ...
    inline bucket *get_bucket(const Key &key, std::size_t &hash_created) const {
        ...
        std::size_t index = hash_created & (capacity - 1);
        ...
        while (probes < capacity) {
            ...
            index = (index + key_probe) & (capacity - 1);
            key_probe++;
            bucket = data + index;
            probes++;
        }
     ...
     */
    inline std::size_t probe(std::size_t hash) const { return 1ULL + hash % (capacity - 1ULL); }
    ...
    inline iterator find(const Key &key) const {
        ...
        std::size_t index = key_hash & (capacity - 1);
        std::size_t probes = 0;
        auto bucket = data + index;
...
/**
 * Useful for fast access to access to the keys in the hashmap and if you only
 * add and do not remove elements.
 */
template <typename Key, typename T, class HashFunc, class CompFunc>
struct hashmap<Key, T, HashFunc, CompFunc, dictionary_allocator<hashmap_bucket<Key, T>>>
    : private _allocator_ebo<hashmap_bucket<Key, T>, dictionary_allocator<hashmap_bucket<Key, T>>> {

    using Allocator = dictionary_allocator<hashmap_bucket<Key, T>>;
    static constexpr float load_factor = 0.75f;
    static constexpr float growth_factor = 1.0f;
    struct iterator {
        hashmap_bucket<Key, T> *pointer;
    ...
    struct key_iterator {
        ...
        inline constexpr key_iterator(Key *ptr, Key *end) : pointer(ptr), end_ptr(end) {
            if (pointer != end_ptr && !pointer)
                operator++();
        }

        inline Key &operator*() const { return *pointer; }

        inline Key *operator->() { return pointer; }

        inline key_iterator &operator++() {
            do {
                ++pointer;
            } while (pointer != end_ptr && !pointer);
            return *this;
        }

        inline key_iterator operator++(int) {
            key_iterator temp = *this;
            do {
                ++pointer;
            } while (pointer != end_ptr && !pointer);
            return temp;
        ...
        friend bool operator==(const key_iterator &a, const key_iterator &b) { return a.pointer == b.pointer; }
        friend bool operator!=(const key_iterator &a, const key_iterator &b) { return a.pointer != b.pointer; }
    };
    ...
    inline hashmap(std::initializer_list<std::pair<Key, T>> init_list) : _allocator_ebo<hashmap_bucket<Key, T>, Allocator>() {
        ...
        capacity = first_power2(init_size * 4);
        data = this->malloc(capacity);
        size = 0;
        std::memset((char *)(void *)data, 0, capacity * bucket_size());
        for (const auto &pair : init_list) add(pair.first, pair.second);
    ...
    inline iterator add_with_hash(const Key &key, const T &value, std::size_t hash) {
        ...
        if (bucket->occupied()) {
            ...
            bucket->_hash = key_hash;
            bucket->_flags = HASHMAP_BUCKET_OCCUPIED;
            new (&Allocator().get_keys(data, capacity)[size]) Key(key);
            size++;
    ...
    inline void _remove_key(const Key &key) {
        auto keys = Allocator().get_keys(data, capacity);
        for (std::size_t i = 0; i < size; i++) {
            if (CompFunc()(keys[i], key)) {
                keys[i].~Key();
                for (std::size_t j = i; j < size - 1; j++) {
                    new (&keys[j]) Key(std::move(keys[j + 1]));
                }
                return;
            }
        }
    ...
    inline void destroy() {
        if (data) {
            ...
            for (std::size_t i = 0; i < size; i++) {
                Allocator().get_keys(data, capacity)[i].~Key();
            }
            std::memset((char *)(void *)data, 0, capacity * bucket_size());
        }
    }
    ...
    inline T &operator[](const Key &key) {
        ...
        bucket->_hash = key_hash;
        bucket->_flags = HASHMAP_BUCKET_OCCUPIED;
        new (&Allocator().get_keys()[size]) Key(key);
        size++;
        return bucket->value;
    }

    inline Key *keys() { return Allocator().get_keys(data, capacity); }

    inline const Key *keys() const { return Allocator().get_keys(data, capacity); }

    inline key_iterator key_begin() {
        return key_iterator(Allocator().get_keys(data, capacity), Allocator().get_keys(data, capacity) + size);
    }

    inline key_iterator key_end() {
        return key_iterator(Allocator().get_keys(data, capacity) + size, Allocator().get_keys(data, capacity) + size);
    }

    inline const key_iterator key_begin() const {
        return key_iterator(Allocator().get_keys(data, capacity), Allocator().get_keys(data, capacity) + size);
    }

    inline const key_iterator key_end() const {
        return key_iterator(Allocator().get_keys(data, capacity) + size, Allocator().get_keys(data, capacity) + size);
    }
};

template <typename Key, typename T>
using dictionary = hashmap<Key, T, hashmap_hash<Key>, hashmap_internal_comp<Key>, dictionary_allocator<hashmap_bucket<Key, T>>>;
